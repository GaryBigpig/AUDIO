...
def audio_path_class(metaFile,audioFileName):
    metadatas = pd.read_csv(metaFile, sep='\t')
    metadata = metadatas[metadatas['filename'] == audioFileName]
    path_name = os.path.join(globle_params.TAU_PATH, audioFileName)
    return path_name, metadata['scene_label'].values[0]
...
def MFCC_Feature(metaFile):
    ...
    for i in range(metadatas.shape[0]):
        file=os.path.join(globle_params.TAU_PATH, metadatas.filename[i])
        class_id = metadatas.scene_label[i]
        try:
...
if __name__ == '__main__':
    ...
    MFCC_Feature(globle_params.TAU_METAFILE)
